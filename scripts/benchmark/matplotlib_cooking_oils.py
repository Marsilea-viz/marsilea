...
keys = ['<150 °C (Dressings)', '150-199 °C (Light saute)',
        '200-229 °C (Stir-frying)', '>230 °C (Deep-frying)', 'Control'][::-1]
keys_text = ["Control", ">230 °C\nDeep-frying", "200-229 °C\nStir-frying",
             "150-199 °C\nLight saute", "<150 °C\nDressings"]
fat_contents = ['saturated', 'polyunsaturated (omega 3 & 6)', 'monounsaturated', 'other fat']

colors = ["#e5e7eb", "#c2410c", "#fb923c", "#fca5a5", "#fecaca"]

height_ratios = [len(groups.get_group(k)) for k in keys]
width_ratios = [.5, 1, 1, 3, .2, 1.5, 1.5]

fig, axes = plt.subplots(nrows=5, ncols=7, figsize=(11, 12), dpi=250,
                         gridspec_kw=dict(height_ratios=height_ratios, width_ratios=width_ratios, hspace=0.04))
...
for irow, (group, axs, c, gt) in enumerate(zip(keys, axes, colors, keys_text)):
    if irow != 4:
        ...
        for ix in [0, 1, 4, 5]:
            axs[ix].set_axis_off()
        for ix in [2, 3, 6]:
            axs[ix].tick_params(left=False, top=False, right=False,
    ...
    if df.shape[0] > 1:
        ...
        plot = dendrogram(Z, ax=axs[0], orientation='left')
    ...
    axs[1].add_artist(bg)
    axs[1].text(x=.9, y=.5, s=gt, ha="right", va="center")
    ...
    for text, loc, c in zip(flavour, text_loc, flavour_colors):
        axs[4].text(x=.5, y=loc, s=text, ha="center", va="center",
                    fontfamily="Font Awesome 6 Free", color=c, transform=axs[4].transAxes)

    # Add Label
    for text, loc in zip(df.index.str.capitalize(), text_loc):
        axs[5].text(x=0, y=loc, s=text, ha="left", va="center", transform=axs[5].transAxes)
...
ax = axes[0, -1]
ax.text(.45, 1, "Omega 3 (%)", transform=ax.transAxes, ha="right", va="bottom", )
ax.text(.55, 1, "Omega 6 (%)", transform=ax.transAxes, ha="left", va="bottom", )
...
for fat, c in zip(fat_contents, [red, yellow, green, gray]):
    ...
    labels.append(fat)
ax.legend(handles=handles, labels=labels, title="Fat Content (%)", ncol=2, alignment="left",
          title_fontproperties=dict(weight=600),
          bbox_to_anchor=(.5, -.5), bbox_transform=ax.transAxes, loc="upper center")
