...
def load_and_process_data():
    """Load and process the cooking oils data."""
    oils = ma.load_data("cooking_oils")
    oils["flavour_colors"] = oils["flavour"].map(lambda x: CMAPPER[x])
    oils["flavour"] = oils["flavour"].map(lambda x: MAPPER[x])
    return oils


def create_cluster_board(oils):
    """Create and configure the ClusterBoard for visualization."""
    # Initialize ClusterBoard and add layers as previously done
    # This function will contain the plotting configurations including
    # creating the stack bar, numbers, center bar, chunk, and adding layers
    fat_content = oils[
    ...
    cb.add_right(
        mp.Labels(
            ...
            text_props={"color": oils["flavour_colors"]},
        )
    )
...
def main():
    ...
    install_fonts()
    oils = load_and_process_data()
    create_cluster_board(oils)
    plt.show()
