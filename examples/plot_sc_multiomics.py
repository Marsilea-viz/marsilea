...
with mpl.rc_context({'font.size': 14}):
    gene_profile = ma.SizedHeatmap(dataset['gene_pct_matrix'], color=dataset['gene_exp_matrix'],
                                   cluster_data=cluster_data, marker="P", cmap="PuRd",
    ...
    gene_profile.add_right(ma.plotter.Chunk(lineage, lineage_colors, padding=10))
    gene_profile.add_dendrogram("right", method="ward", colors=lineage_colors, meta_color="#451952", linewidth=1.5)
    gene_profile.add_bottom(ma.plotter.Labels(marker_names, color='#392467'))
    ...
    gene_profile.add_left(ma.plotter.Labels(display_cells, text_props={'color': [m[c] for c in lineage_cells]},
    ...
    protein_profile = ma.SizedHeatmap(dataset['protein_pct_matrix'], color=dataset['protein_exp_matrix'],
                                      cluster_data=cluster_data, marker="*", cmap="YlOrBr",
                                      color_legend_kws={'title': 'Mean Expression\n(Protein)'},
                                      size_legend_kws={'colors': '#de600c', 'fmt': fmt,
                                                       'title': '% expression in group'})
    protein_profile.hsplit(labels=lineage_cells, order=lineage)
    protein_profile.add_bottom(ma.plotter.Labels(marker_names, color="#E36414"))
    protein_profile.add_dendrogram("right", method="ward", show=False)
    ...
    protein_profile.add_left(ma.plotter.Numbers(cells_count, color="#B80000"), pad=.1)
    protein_profile.add_title("Proteomics Profile", fontsize=16)

    comb = (protein_profile + gene_profile)
    comb.add_legends("left")
    comb.render()
